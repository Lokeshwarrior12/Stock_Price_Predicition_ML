...
class HybridEnsemble:
    ...
    def predict(self, X_seq, X_tabular):
        ...
        # XGBoost predictions
        if self.xgb_model:
            xgb_preds = self.xgb_model.predict(xgb.DMatrix(X_tabular))
...
if __name__ == "__main__":
    ...
    X_seq = np.random.rand(100, 30, 10)   # (samples, timesteps, features)
    X_tab = np.random.rand(100, 20)       # tabular data (fundamentals/sentiment)
    y_true = np.random.rand(100)

    ensemble = HybridEnsemble(weights=[0.6, 0.4])
    ensemble.load_lstm("models/checkpoints/lstm_model.pth", input_dim=10)
